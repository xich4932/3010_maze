...
std::vector<int> getDirection(int point, int max_c, int max_r, const std::vector<int> path){
    ...
    for(int d = 0; d < ret.size(); d++){
        if(checkInPath(ret[d], path)){
            ret.erase(std::ret.remove(ret.begin(), ret.end(), ret[d]), ret.end());
        }
    }
    return ret;
}
