...
std::vector<int> getDirection(int point, int max_c, int max_r, int arr[]){
    ...
    for(int i = 0; i < tmp.size(); i++){
        if(arr[ret[i]]) tmp.erase(tmp.begin() + i);
    }
    printer(tmp);
    return tmp;
}
...
bool Board::generate(){ 
    ...
    int time = 16;
     while( path[path.size()-1] != 15 && path.size() < 13 && max_step < 16){
        std::vector<int> direction = getDirection(path[path.size()-1], cols_, rows_, len);
        std::cout << "direction : ";
        printer(direction);
