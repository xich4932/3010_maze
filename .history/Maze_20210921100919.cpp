...
std::vector<int> getDirection(int point, int max_c, int max_r){
...
bool Board::generate(){ 
    ...
    int end_r = rows_ - 1;
    int time = 16;
     while( path[path.size()-1] != 15 && path.size() < 13 && max_step < 100){
        std::vector<int> direction = getDirection(path[path.size()-1], cols_, rows_);
        printer(direction);
        std::cout << direction.size() << std::endl;
        int curr = rand()%direction.size();
        
        printer(path);
        int temp_r = direction[curr] % rows_;
        int temp_c = direction[curr] / rows_;
        std::cout <<"step " << max_step << std::endl;
        max_step ++;
        vec_push_back(4*temp_c + temp_r, path);
    } 
