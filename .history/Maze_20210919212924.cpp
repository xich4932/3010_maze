...
bool checkInPath(const int num, const vector<int> &vec){
...
bool generate(Board* start_board){
    ...
    while(start_r != end_r && start_c != end_c && max_step < 9){
        std::vector<std::array<int,2>> direction = getDirection(0,0, start_board->get_rows(), start_board->get_cols());
